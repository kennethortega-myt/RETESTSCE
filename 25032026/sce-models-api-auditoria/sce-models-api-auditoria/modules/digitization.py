...
def procesar_control_automatico(acta_id: int, usuario: str, codigo_cc: str, abrev_proceso: str, mensaje_ok: bool, file_escrutinio: int, file_instalacion_sufragio: int):
    ...
    try:
        ...
        logger.exception(f"Timeout al llamar al servicio: {url}", queue=constantes.QUEUE_LOGGER_VALUE_VALIDATE)
        raise
    except requests.RequestException as e:
        logger.exception(f"Error en la petición HTTP: {str(e)}", queue=constantes.QUEUE_LOGGER_VALUE_VALIDATE)
        raise
    except Exception as e:
