...
def generate_cortes_1(img_name, cortes, is_convencional, usuario, cod_centro_computo):
    file_name_temp = os.path.abspath(img_name)
    file_name = os.path.basename(file_name_temp)
    file_path, _, _ = download_file_from_dir_and_align(file_name, False, is_convencional=is_convencional, log_queue=constantes.QUEUE_LOGGER_VALUE_MIEMBROS_MESA)
    ...
    try:
        ...
        rectangles = actasmodel.extract_rectangles(image_loader, rectangles_named, acta_observada = False, is_convencional=is_convencional, square_coords_alineados = None, acta_type = None, codigo_eleccion = None,log_queue=constantes.QUEUE_LOGGER_VALUE_MIEMBROS_MESA)
    ...
    info = VotesPrefImageProcessor(rec_loader).get_data_secciones_mm_1()
...
def generate_cortes_2(img_name, cortes, is_convencional, usuario, cod_centro_computo):
    ...
    try:
        ...
        rectangles = actasmodel.extract_rectangles(image_loader, rectangles_named, acta_observada = False, square_coords_alineados = None, is_convencional=is_convencional, acta_type = None, codigo_eleccion = None, log_queue=constantes.QUEUE_LOGGER_VALUE_MIEMBROS_MESA)
        rect_dict = dict(rectangles)
...
def get_secciones(mesa_id, abreviatura, contexto_mm, usuario, cod_centro_computo):
    ...
    secciones_id_1 = generate_cortes_1(img1, cortes_mm, is_convencional, usuario, cod_centro_computo)
