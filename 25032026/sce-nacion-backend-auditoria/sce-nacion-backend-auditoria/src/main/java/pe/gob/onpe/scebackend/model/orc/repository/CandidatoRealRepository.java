...
import pe.gob.onpe.scebackend.model.orc.entities.CandidatoReal;

public interface CandidatoRealRepository extends JpaRepository<CandidatoReal, Integer> {

	@Query(value = "SELECT DISTINCT c.* " +
			"FROM mae_candidato_real c " +
			"INNER JOIN mae_agrupacion_politica_real map2 " +
			"ON map2.n_agrupacion_politica_real_pk = c.n_agrupacion_politica " +
			"INNER JOIN det_ubigeo_eleccion_agrupacion_politica ag " +
			"ON ag.n_agrupacion_politica = map2.n_agrupacion_politica_real_pk " +
	...
	public List<CandidatoReal> findByCc(String codigo);
