...
public class ReporteDetalleAvanceRegistroUbigeoMiembroMesaService implements IReporteDetalleAvanceRegistroUbigeoMiembroMesaService {
    ...
    public byte[] reporte(ReporteDetalleAvanceRegistroUbigeoRequestDto filtro, String authorization) throws JRException, SQLException {
        try {
            ...
            List<ReporteDetalleAvanceRegistroUbigeoDto> lista = reporteDetalleAvanceRegistroUbigeoRepository.listarReporteDetalleAvanceRegistroUbigeoMiembroMesa(filtro);

            lista.forEach(item -> {
                if (item.getCodigoUbigeo() != null) {

                    String ubigeoStr = String.valueOf(item.getCodigoUbigeo()).trim();
                    try {
                        item.setCodigoUbigeo(String.format("%06d", Integer.parseInt(ubigeoStr)));
                    } catch (NumberFormatException e) {
                        item.setCodigoUbigeo(ubigeoStr);
                    }
                }
                if (item.getCodigoAmbitoElectoral() != null) {
                    item.setCodigoAmbitoElectoral(String.valueOf(item.getCodigoAmbitoElectoral()).trim());
                }
                if (item.getCodigoCentroComputo() != null) {
                    item.setCodigoCentroComputo(String.valueOf(item.getCodigoCentroComputo()).trim());
                }
                if (item.getMesa() != null) {
                    item.setMesa(String.valueOf(item.getMesa()).trim());
                }
            });
            ...
            InputStream imagen = this.getClass().getClassLoader().getResourceAsStream(ConstantesComunes.PATH_IMAGE_COMMON_NAC + ConstantesComunes.NOMBRE_LOGO_ONPE);
            ...
            this.logService.registrarLog(usuarioLogin,ConstantesComunes.LOG_TRANSACCIONES_TIPO_REPORTE, this.getClass().getSimpleName(), "Se consultó el Reporte Detalle de Avance de Registros por Ubigeo Miembros de Mesa",
    ...
    private String obtenerUbigeo(String ubigeoNivelUno,String ubigeoNivelDos,String ubigeoNivelTres) {
        ...
        return ubigeRetorno != null ? String.valueOf(ubigeRetorno).trim() : ConstantesReportes.CODIGO_UBIGEO_NACION;
    }
}
