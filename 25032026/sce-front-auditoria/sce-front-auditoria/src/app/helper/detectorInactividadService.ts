...
export class DetectorInactividadService{
  ...
  private inactivityHandler: (() => void) | null = null;
  ...
  public startMonitoring(): void {
    this.ngZone.runOutsideAngular(() => {
      this.inactivityHandler = () => this.resetTimeout();
      const events = ['mousemove', 'keydown', 'click', 'touchstart'];
      events.forEach(event => window.addEventListener(event, this.inactivityHandler));
    });
  ...
  public stopMonitoring(): void {
    ...
    if (this.inactivityHandler) {
      this.ngZone.runOutsideAngular(() => {
        const events = ['mousemove', 'keydown', 'click', 'touchstart'];
        events.forEach(event => window.removeEventListener(event, this.inactivityHandler!));
      });
