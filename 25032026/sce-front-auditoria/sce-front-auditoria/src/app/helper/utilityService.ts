...
export class UtilityService{
  ...
  public abrirModalActaPorId(
    ...
    if (!actaId || actaId <= 0) {
      this.mensajePopup(titulo || 'Error', 'No se pudo identificar el acta a visualizar.', IconPopType.ALERT);
      return;
    }

    if (!titulo) {
      console.warn('abrirModalActaPorId: el parámetro "titulo" está vacío.');
      titulo = '';
    }

    if (!destroyRef) {
      console.error('abrirModalActaPorId: "destroyRef" es requerido para gestionar la suscripción.');
      return;
    }
  ...
  private obtenerBlobsYMostrarModal(
    ...
    forkJoin(requests)
      ...
      .subscribe({
        next: (responses) => {
          const todosConData = responses.every(r => !!r?.data);
          if(!todosConData){
            this.setLoading(false);
            this.mensajePopup(titulo, 'No se pudieron obtener todos los archivos del acta.', IconPopType.ALERT);
            return;
          }

          const blobs = responses.map(r => Utility.base64toBlob(r.data, 'application/pdf'));

          const todosConContenido = blobs.every(b => b.size > 0);
          if (!todosConContenido) {
            this.setLoading(false);
            this.mensajePopup(titulo, 'Uno o más archivos del acta están vacíos.', IconPopType.ALERT);
            return;
          }
