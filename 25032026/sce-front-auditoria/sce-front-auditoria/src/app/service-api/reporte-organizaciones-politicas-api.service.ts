...
export class ReporteOrganizacionesPoliticasApiService {
  ...
  getReporteOrganizacionesPoliticasCc(filtros: FiltroOrganizacionesPoliticas):Observable<GenericResponseBean<string>>{
    return this.httpClient.post<GenericResponseBean<string>>(
      this.urlServidorOrc + Constantes.CB_REPORTE_ORGANIZACIONES_POLITICAS_POST_BASE64, filtros,
      {
  ...
  getReporteOrganizacionesPoliticasNacion(filtros: FiltroOrganizacionesPoliticas, acronimo: string):Observable<GenericResponseBean<string>>{
    ...
    const url = this.globalService.reemplazarDobleSlash(`${this.urlServidor}/${Constantes.CB_REPORTE_ORGANIZACIONES_POLITICAS_POST_BASE64}`);
