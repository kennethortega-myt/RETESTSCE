...
import { AgrupacionPolitica } from "../model/control-calidad/AgrupacionPolitica";
...
export class ControlCalidadApiService {
  ...
  private readonly urlServidor: string;
    ...
    obtenerAgrupacionesPoliticas(idActa: number): Observable<GenericResponseBean<AgrupacionPolitica[]>>{
      return this.httpClient.get<GenericResponseBean<AgrupacionPolitica[]>>(
        `${this.urlServidor}${Constantes.CB_CONTROL_CALIDAD_ORGANIZACIONES}${idActa}`,        
        {
          headers:  { 'Authorization': "Bearer " + this.auth.getCToken()}
        });
    }
