...
export class FiltroReporteModeloUnoComponent implements OnInit, OnDestroy {
  ...
  consultar(): void {
    ...
    this.filtroReporteModeloUno.nombreUbigeoUno = this.labelNivelUbigeoUno;
    this.filtroReporteModeloUno.nombreUbigeoDos = this.labelNivelUbigeoDos;
    this.filtroReporteModeloUno.nombreUbigeoTres = this.labelNivelUbigeoTres;

    this.eventBuscar.emit(this.filtroReporteModeloUno);
  }

  get labelNivelUbigeoUno(): string {
      const seleccionado = this.form.get('nivelUbigeoUno')?.value.codigo;
