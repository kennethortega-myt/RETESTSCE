...
import {
  AvanceDigitalizacionDenunciaService
} from '../../../../../service/reporte/avance-digitalizacion-denuncia.service';

@Component({
  selector: 'app-avance-digitalizacion-denuncias',
  templateUrl: './avance-digitalizacion-denuncias.component.html'
})
export class AvanceDigitalizacionDenunciasComponent extends AuthComponent implements OnInit {
  ...
  private readonly tituloAlert = "Avance Digitalización de Denuncias";
  private readonly destroyRef: DestroyRef = inject(DestroyRef);

  constructor(
    private readonly monitoreoService: MonitoreoNacionService,
    private readonly utilityService: UtilityService,
    private readonly avanceDigitalizacionDenunciaService: AvanceDigitalizacionDenunciaService,
  ...
  buscarReporte(): void {
    ...
    const ubigeo1 = this.listNivelUbigeoUno?.find(ubi => ubi.id === this.form.get('nivelUbigeoUno').value);
    filtro.departamento = this.form.get('nivelUbigeoUno').value === '0' ? 'TODOS' :
                        `${ubigeo1.codigo} ${ubigeo1.nombre}`;

    const ubigeo2 = this.listNivelUbigeoDos?.find(ubi => ubi.id === this.form.get('nivelUbigeoDos').value);
    filtro.provincia = this.form.get('nivelUbigeoDos').value === '0'  ? 'TODOS' :
                        `${ubigeo2.codigo} ${ubigeo2.nombre}`;

    const ubigeo3 = this.listNivelUbigeoTres?.find(ubi => ubi.id === this.form.get('nivelUbigeoTres').value);
    filtro.distrito = this.form.get('nivelUbigeoTres').value === '0' ? 'TODOS' :
                        `${ubigeo3.codigo} ${ubigeo3.nombre}`;
    ...
    this.avanceDigitalizacionDenunciaService.getReporteAvanceDigitalizacionDenunciaNacion(filtro, acronimo)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
