...
import { ReporteMesasUbigeoNacionService } from 'src/app/service/reporte-mesas-ubigeo-nacion.service';
import { descargarPdf, generarPdf } from 'src/app/transversal/utils/funciones';
import { FiltroMesaPorUbigeoNacion } from 'src/app/model/reportes/filtroMesaPorUbigeoNacion';
import { Utility } from 'src/app/helper/utility';
import { NivelUbigeo } from 'src/app/model/enum/nivel-ubigeo.enum';


@Component({
  selector: 'app-mesas-ubigeo-nacion',
  templateUrl: './mesas-ubigeo-nacion.component.html',
})
export class MesasUbigeoNacionComponent extends AuthComponent implements OnInit {
  ...
  private readonly tituloAlert = "Mesas por Ubigeo";
  ...
  constructor(
    ...
    private readonly reporteMesasUbigeoNacionService: ReporteMesasUbigeoNacionService,
  ...
  buscarReporte(): void {
    ...
    let filtros: FiltroMesaPorUbigeoNacion = new FiltroMesaPorUbigeoNacion();
    ...
    filtros.tipoDocumento = 1;
    filtros.centroComputo = this.form.get('centroComputo').value.id === 0 ? null : this.form.get('centroComputo').value.codigo;
    filtros.departamento = this.form.get('nivelUbigeoUno').value;
    filtros.provincia = this.form.get('nivelUbigeoDos').value;
    filtros.distrito = this.form.get('nivelUbigeoTres').value;
    ...
    this.reporteMesasUbigeoNacionService.getReporteMesasUbigeoNacionPdf(filtros, proceso.acronimo)
    .pipe(takeUntilDestroyed(this.destroyRef))
    .subscribe({
      next: (response) => {
      ...
      error: () => {
        this.isShowReporte = false;
        this.utilityService.mensajePopup(this.tituloAlert, "No fue posible obtener el reporte de mesas por ubigeo.", IconPopType.ERROR);
      }
    })
  ...
  imprimirReporte() {
    descargarPdf(this.pdfBlob, 'ListadoMesasUbigeo.pdf');
  }
