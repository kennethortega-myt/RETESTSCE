...
export class AuditoriaDigitacionNacionComponent extends AuthComponent implements OnInit {
  ...
  public tituloAlert = "Reporte de Verificación de Digitación";
  public mensaje: string = this.mensajeDefault;
  constructor(private readonly formBuilder: FormBuilder,
              private readonly monitoreoService: MonitoreoNacionService,
              private readonly utilityService: UtilityService,                          
              private readonly reporteAuditoriaDigitacionService: ReporteAuditoriaDigitacionService,
  ) {
    super();
  ...
  getReporteAvanceMesaPdfIncorrecto(error: any) {
    this.isShowReporte = false;
    sessionStorage.setItem('loading', 'false');
    this.utilityService.mensajePopup(this.tituloAlert, "No fue posible obtener el reporte Verificación de Digitación del Acta.", IconPopType.ERROR);
  }
  ...
  sonValidosLosDatosMinimos(): boolean {
    ...
    //Por rendimiento, se debe seleccionar el dpto
    if(this.form.get('nivelUbigeoUno').value.toString() === '0' && !Utility.listaSoloExtranjero(this.listNivelUbigeoUno)){
      this.utilityService.mensajePopup(this.tituloAlert, `Seleccione ${this.labelNivelUbigeoUno}.`, IconPopType.ALERT);
      return false;
    }

    //Por rendimiento, se debe seleccionar provincia como mínimo
    if(this.form.get('nivelUbigeoDos').value.toString() === '0' && !Utility.listaSoloExtranjero(this.listNivelUbigeoUno)){
