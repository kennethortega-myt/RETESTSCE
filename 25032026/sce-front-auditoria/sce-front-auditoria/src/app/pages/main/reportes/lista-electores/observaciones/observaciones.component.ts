...
import {ReporteMesasObservacionesService} from '../../../../../service/reporte/reporte-mesas-observaciones.service';
...
export class ObservacionesComponent extends AuthComponent implements OnInit {
  ...
  private readonly tituloAlert = "Mesas con Observaciones";
  ...
  constructor(
    ...
    private readonly reporteMesasObservacionesService: ReporteMesasObservacionesService,
  ...
  buscarReporte(): void {
    ...
    filtro.esquema = proceso.nombreEsquemaPrincipal
    filtro.idProceso = proceso.id;
    ...
    filtro.proceso = proceso.nombre;
    filtro.usuario = this.usuario.nombre;

    this.isShowReporte = true;
    sessionStorage.setItem('loading','true');

    this.reporteMesasObservacionesService.getReporteMesasObservacionesNacion(filtro, proceso.acronimo)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
