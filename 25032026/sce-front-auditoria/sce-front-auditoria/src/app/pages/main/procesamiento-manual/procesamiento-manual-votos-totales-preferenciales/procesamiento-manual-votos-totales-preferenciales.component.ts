...
extends ProcesamientoManualBaseComponent implements OnInit {
  ...
  protected override onDoubleClick(event: MouseEvent): void {
    ...
    const voto: VerificationVoteItemBean | VerificationVotePreferencialItemBean = colIndex === undefined ?  agrupacion: agrupacion.votoPreferencial[colIndex];
  ...
  protected override deshabilitarFormulario(): void {
    const paramsTotal = this.formGroupSeccionVoto.get('params') as FormGroup;
    Object.keys(paramsTotal.controls).forEach(key => {
      paramsTotal.get(key)?.setValue('');
      paramsTotal.get(key)?.disable();
    });
