...
implements OnInit{
  ...
  public ngOnInit(): void {
    ...
    this.messageProcesamientoManualService.getFechaProceso()
      .pipe(take(1))
      .subscribe(fecha => {
        if (fecha) this.actualizarFechaProceso(fecha);
      });
  }
  ...
  private actualizarFechaProceso(fecha: Date): void {
    ...
    this.formGroupIntalacion.get('fechaInstalacion')?.setValue(this.fechaProcesoDate);
    this.formGroupEscrutinio.get('fechaEscrutinio')?.setValue(this.fechaProcesoDate);
  }
  ...
  public changeCheckSolNulidad(event: Event): void {
    this.actaParaProcesamientoManualBean.observationSection.nullityRequest = (event.target as HTMLInputElement).checked;
  }
  ...
  onScroll(container: string): void {
    const leftContainer1 = document.getElementById('left-container');
    const rightContainer1 = document.getElementById('right-container');

    if (container === 'left' && !this.isSyncingRight) {
      this.isSyncingLeft = true;
      if (rightContainer1) rightContainer1.scrollTop = leftContainer1?.scrollTop || 0;
    } else if (container === 'right' && !this.isSyncingLeft) {
      this.isSyncingRight = true;
      if (leftContainer1) leftContainer1.scrollTop = rightContainer1?.scrollTop || 0;
    }
  ...
  protected onInputVoto(fileId: number, preferencial: boolean, event: Event, i: number,j ?:number): void {
    const inputContrl = event.target as HTMLInputElement;
    const value = inputContrl.value;
    ...
    if (value !== validado) {
      ...
      setTimeout(() => {
        inputContrl.setSelectionRange(validado.length, validado.length);
      });
    }
  }
  ...
  protected getControlName(rowIndex: number, colIndex?: number): string {
    ...
    return colIndex === undefined ? `${fileId}-${rowIndex}`: `${fileId}-${rowIndex}-${colIndex}`;
  }
  ...
  public changeVoto(fileId:number,preferencial: boolean ,i:number,j ?:number){
    ...
    const controlVoto = formGroup.get("params")?.get(controlName);

    if (!controlVoto) return;

    let value = (controlVoto.value ?? '').toString();
    const validado= this.validateInput(value);

    if (value !== validado) {
      controlVoto?.setValue(validado);
      value = validado;
    }
