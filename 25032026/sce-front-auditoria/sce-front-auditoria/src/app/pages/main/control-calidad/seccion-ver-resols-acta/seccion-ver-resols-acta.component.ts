...
export class SeccionVerResolsActaComponent implements OnChanges {
  ...
  cargarResolucionesFiles() {
    const observablesResol :Observable<any>[] = this.listaResoluciones.map(resol => this.resolucionService.getFileV3(resol.idArchivo).pipe(catchError( () => of(null))));
