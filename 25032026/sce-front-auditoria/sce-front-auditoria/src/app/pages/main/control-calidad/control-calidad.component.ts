...
export class ControlCalidadComponent implements OnInit {
  ...
  validarRevision(): boolean {
    ...
    if(this.listaResoluciones.length > 0 && this.listaResoluciones.length !== this.uniqueSetIdsResolRevisados.size) {
      ...
      const resolFaltante = this.listaResoluciones
                        .find(resol => !this.uniqueSetIdsResolRevisados.has(resol.idResolucion));

      this.utilityService.mensajePopup(this.tituloMenu,
              `Se debe realizar el control de calidad de la Resolución ${resolFaltante.nombreResolucion}.\n
  ...
  public actaDescripcion(acta: ActaPendienteControlCalidad): string {
    if(!acta?.copia) {
      return acta.mesa;
    }
    return `${acta.mesa} - ${acta.copia}${acta.digitoChequeo}`;
  }
  ...
  get disableButtonRechazar() {
    return !(this.listaResoluciones && this.listaResoluciones.length > 0);
  }

  get numeroActaCompleto(): string {
    if(this.actaPendienteSeleccionada.copia && this.actaPendienteSeleccionada.digitoChequeo) {
    ...
    return `${this.actaPendienteSeleccionada.mesa}`;
  }
