...
export class HabilitarActasStaeComponent implements AfterViewInit{
  ...
  isAgregarDisabled: boolean = false;
  ...
  agregarMesa() {
    // Evitar múltiples ejecuciones simultáneas - PRIMERO verificar el bloqueo
    if (this.isAgregarDisabled) {
      return;
    }

    // Bloquear INMEDIATAMENTE antes de leer el input
    this.isAgregarDisabled = true;

    // Leer y limpiar el input
    const numMesa = this.formHabilitar.get('numMesaFormControl')?.value?.trim();
    this.formHabilitar.get('numMesaFormControl')?.setValue('');

    // Validar el valor que se leyó
    if (!numMesa?.length || numMesa.length !== Constantes.LONGITUD_MESA) {
      // Si la validación falla, desbloquear y NO mostrar mensaje si el input estaba vacío
      this.isAgregarDisabled = false;
      if (numMesa && numMesa.length > 0) {
  ...
  private validaHabilitarActasStaeCorrecto(response: GenericResponseBean<Array<ActaBean>>) {
    ...
    if (newItems.length === 0) {
      this.utilityService.mensajePopup(this.tituloAlert, "No existen actas para agregar.", IconPopType.ALERT);
      return;
    }
