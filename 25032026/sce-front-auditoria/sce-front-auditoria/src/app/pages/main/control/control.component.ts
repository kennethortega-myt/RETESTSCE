...
import { PopControlComponent } from './pop-control/pop-control.component';
...
export class ControlComponent implements OnInit, OnDestroy {
  ...
  totalActasPendiente:string;
  totalActasAprobadas: string;
  totalActasRechazadas: string;
  totalActasDigitalizadas:string;
  totalActasNoInstaladosExtSin:string;
  ...
  isVisibleBtnDigitacionManual: boolean;
  ...
  constructor(
    ...
    this.totalActasAprobadas = Utility.rellenarCerosAIzquierda(0,4);
    this.totalActasRechazadas = Utility.rellenarCerosAIzquierda(0,4);
    this.totalActasDigitalizadas = Utility.rellenarCerosAIzquierda(0,4);
    this.totalActasPendiente = Utility.rellenarCerosAIzquierda(0,4);
    this.totalActasNoInstaladosExtSin = Utility.rellenarCerosAIzquierda(0,4);
    this.nroMesa = "000000";
  ...
  openDialogCtrl(tipoActa:string) {
    ...
    const dialogRef = this.dialog.open(PopControlComponent, {
      width: '1360px',
      maxWidth: '98%',
      disableClose: true,
      data:{
  ...
  verActa(acta: DigitizationListActasItemBean){
    ...
    this.actualizarBotones(this.actaSeleccionada.acta1Status, this.actaSeleccionada.acta2Status, this.actaSeleccionada.estado);
  ...
  actualizarBotones(acta1Status: string, acta2Status: string, estado: string){
    ...
    if(estado == Constantes.CE_ESTADO_MESA_DIGITALIZADO) {

      if(acta1Status==Constantes.CE_ESTADO_ACTA_PARA_PROCESAMIENTO_MANUAL || acta2Status==Constantes.CE_ESTADO_ACTA_PARA_PROCESAMIENTO_MANUAL){
        this.isVisibleBtnAprobar = false;
        this.isVisibleBtnDigitacionManual = true;
        this.isVisibleBtnRechazar = true;
      } else {
        this.isVisibleBtnAprobar = true;
        this.isVisibleBtnDigitacionManual = false;
        this.isVisibleBtnRechazar = true;
      }
  ...
  confirmarAprobarMesa(): void {
    ...
    let observadoModelo = this.actaSeleccionada.acta1Status == Constantes.CE_ESTADO_ACTA_REDIGITALIZAR;

    let mensaje = `¿Está seguro de continuar?`;

    if(observadoModelo){
      mensaje = `El acta digitalizada tiene observaciones detectadas. Es posible que algunos cortes de la imagen no se procesen correctamente. ¿Desea continuar?`
    }
  ...
  aprobarMesa(){
    ...
    this.controlDigitalizacionService.aprobarMesa(this.codigoEleccion,this.aprobarMesaBean)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
  ...
  async cargarDocumentos(){
    ...
    // Cargar resumen solo si no está visible aún
    if(!this.isVisibleResumen){
      this.cargarResumen();
    }
    ...
    if (this.listaActas.length===0){
      ...
      // No llamar a limpiarDatos() para mantener el resumen visible
      ...
      this.isVisibleBtnCargarDocumentos = true;
      // No mostrar botones hasta que se seleccione un acta
      this.isVisibleBtnAprobar = false;
      this.isVisibleBtnRechazar = false;
      this.isVisibleBtnDigitacionManual = false;
    }
  ...
  async obtenerListaActasAsync(){
    try {
      this.listaActas = await firstValueFrom(this.controlDigitalizacionService.obtenerActas(this.codigoEleccion));
    }catch (error){
  ...
  cargarResumen(): void{
    this.controlDigitalizacionService.obtenerResumen(this.codigoEleccion)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
  ...
  obtenerResumenCorrecto(response: DigitizationSummaryResponseBean){
    this.totalActasAprobadas = Utility.rellenarCerosAIzquierda(response.approved,4);
    this.totalActasRechazadas = Utility.rellenarCerosAIzquierda(response.rejected,4);
    this.totalActasDigitalizadas = Utility.rellenarCerosAIzquierda(response.digitalizados,4);
    this.totalActasPendiente = Utility.rellenarCerosAIzquierda(response.pending,4);
    this.totalActasNoInstaladosExtSin = Utility.rellenarCerosAIzquierda(response.noInstaladosExtSin,4);
  ...
  compararPorEstadoYMesa(a:DigitizationListActasItemBean, b:DigitizationListActasItemBean): number{
    ...
    const ordenMesaA = Number(a.mesa);
    const ordenMesaB = Number(b.mesa);
  ...
  rechazarMesa(){
    ...
    this.controlDigitalizacionService.rechazarMesa(this.codigoEleccion,this.rechazarMesaBean)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
  ...
  procesarDigitacionManual(): void {
    let observadoModelo = this.actaSeleccionada.acta1Status == Constantes.CE_ESTADO_ACTA_PARA_PROCESAMIENTO_MANUAL ||
    this.actaSeleccionada.acta2Status == Constantes.CE_ESTADO_ACTA_PARA_PROCESAMIENTO_MANUAL;

    let mensaje = `¿Está seguro de continuar?`;

    if(observadoModelo){
      mensaje = `No fue posible obtener los cortes del acta. Esta pasará por un procesamiento manual. ¿Desea continuar?`
    }
  ...
  /*obsInstalacion() {
    this.visibleInstalacionObs = !this.visibleInstalacionObs;
  }

  cancelarObsInstalacion() {
    this.visibleInstalacionObs = !this.visibleInstalacionObs;
    this.txtObservacionAI = "";
  }*/
