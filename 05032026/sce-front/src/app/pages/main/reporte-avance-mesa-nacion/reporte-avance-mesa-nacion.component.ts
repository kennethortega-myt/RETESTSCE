...
export class ReporteAvanceMesaNacionComponent extends AuthComponent implements OnInit, OnDestroy, AfterViewInit {

  @ViewChild('idDivImagen', { static: true }) midivReporte: ElementRef;
  ...
  getReporteAvanceMesaPdfCorrecto(response: GenericResponseBean<string>) {
    if (response.success) {
      var e1 = document.getElementById("idDivImagen");      //e.firstElementChild can be used.
      ...
      this.pdfBlob = this.base64toBlob(response.data, 'application/pdf')
  ...
  ngAfterViewInit() {
    /*if (this.midivReporte){
      const midiv = this.midivReporte.nativeElement;
    }*/
  }
  ...
  cargarDepartamentos() {
    ...
    this.formGroupReporte.get('departamentoFormControl').setValue(null);
    this.formGroupReporte.get('provinciaFormControl').setValue(null);
    this.formGroupReporte.get('distritoFormControl').setValue(null);
    if (+this.formGroupReporte.get('eleccionFormControl').value.id > 0) {
      this.monitoreoService.obtenerDepartamentosNacion(this.formGroupReporte.get('eleccionFormControl').value.id, this.formGroupReporte.get('procesoFormControl').value.acronimo).subscribe((response) => {
  ...
  cargarProvincias() {
    ...
    this.formGroupReporte.get('provinciaFormControl').setValue(null);
    this.formGroupReporte.get('distritoFormControl').setValue(null);
    if (+this.formGroupReporte.get('departamentoFormControl').value > 0) {
      this.monitoreoService.obtenerProvinciasNacion(this.formGroupReporte.get('departamentoFormControl').value,
        this.formGroupReporte.get('eleccionFormControl').value.id,
        this.formGroupReporte.get('procesoFormControl').value.acronimo,
        this.formGroupReporte.get('proceso').value.nombreEsquemaPrincipal,
        this.formGroupReporte.get('ambitoElectoral').value.id
  ...
  cargarDistritos() {
    ...
    this.listDistrito = [];
    this.formGroupReporte.get('distritoFormControl').setValue(null);
    if (+this.formGroupReporte.get('provinciaFormControl').value > 0) {
      this.monitoreoService.obtenerDistritosNacion(this.formGroupReporte.get('provinciaFormControl').value,
        this.formGroupReporte.get('eleccionFormControl').value.id,
        this.formGroupReporte.get('procesoFormControl').value.acronimo,
        this.formGroupReporte.get('proceso').value.nombreEsquemaPrincipal,
        this.formGroupReporte.get('ambitoElectoral').value.id
