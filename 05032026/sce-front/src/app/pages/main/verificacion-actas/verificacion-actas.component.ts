...
export class VerificacionActasComponent implements OnInit, OnDestroy{
  ...
  imagenesCargadasVote = false;
  imagenesCargadasDate = false;
  imagenesCargadasSign = false;

  contadorEnter: number = 0;
...
// Limpia URLs de PNG en la sección de observaciones
      ...
      eleccionControl.valueChanges
        ...
        .subscribe(value => {
          ...
          if (value && value !== '0') {
            ...
            this.imagenesCargadasVote = false;
  ...
  reiniciarValores(){
    ...
    this.imagenesCargadasVote = false;
    this.imagenesCargadasDate = false;
    this.imagenesCargadasSign = false;
    this.sinDatos = false;
  ...
  async goToPaso1(): Promise<void>{
    try {
      if (!this.imagenesCargadasSign) {
        await Promise.all([
          this.loadSignSectionImages(),
          this.loadObservationSectionImages()
        ]);
        this.imagenesCargadasSign = true;
      }
  ...
  async loadSignSectionImages() {
    const signItems = [
      this.acta.signSection.countPresident,
      this.acta.signSection.countSecretary,
      this.acta.signSection.countThirdMember,
      this.acta.signSection.installPresident,
      this.acta.signSection.installSecretary,
      this.acta.signSection.installThirdMember,
      this.acta.signSection.votePresident,
      this.acta.signSection.voteSecretary,
      this.acta.signSection.voteThirdMember
    ];
    await this.loadImages(signItems);
  }
    ...
    const imagePromises = items.map(async (item) => {
      if (item) {
        try {
          ...
          console.error(`Error al cargar imagen para fileId ${item.fileId}`, error);
          item.filePngUrl = URL.createObjectURL(noImageFirmaBlob);
        }
      }
    });
