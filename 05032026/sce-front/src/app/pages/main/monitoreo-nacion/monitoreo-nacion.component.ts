...
export class MonitoreoNacionComponent {
  ...
  cargarListadoActas() {
    ...
    this.monitoreoService.obtenerActasNacion(params)
      .pipe(takeUntil(this.destroy$)).subscribe((response) => {
        ...
        this.totalActas = this.listaActas.total != null ? Utility.rellenarCerosAIzquierda(this.listaActas.total, 4) : Utility.rellenarCerosAIzquierda(0, 4);
        this.totalActasNormales = this.listaActas.totalNormales != null ? Utility.rellenarCerosAIzquierda(this.listaActas.totalNormales, 4) : Utility.rellenarCerosAIzquierda(0, 4);
        this.totalActasObservadas = this.listaActas.totalObservadas != null ? Utility.rellenarCerosAIzquierda(this.listaActas.totalObservadas, 4) : Utility.rellenarCerosAIzquierda(0, 4);
        this.totalActasEnviadasJne = this.listaActas.totalEnviadasJne != null ? Utility.rellenarCerosAIzquierda(this.listaActas.totalEnviadasJne, 4) : Utility.rellenarCerosAIzquierda(0, 4);
        this.totalActasDevueltasJne = this.listaActas.totalDevueltasJne != null ? Utility.rellenarCerosAIzquierda(this.listaActas.totalDevueltasJne, 4) : Utility.rellenarCerosAIzquierda(0, 4);
        sessionStorage.setItem("loading", 'false');
      }, error => {
        sessionStorage.setItem("loading", 'false');
      });
  ...
  get labelNivelUbigeoUno(): string {
    return Utility.labelFiltroUbigeo(this.departamentoFormControl.value, NivelUbigeo.UNO);
  }

  get labelNivelUbigeoDos(): string {
    return Utility.labelFiltroUbigeo(this.departamentoFormControl.value, NivelUbigeo.DOS);
  }

  get labelNivelUbigeoTres(): string {
    return Utility.labelFiltroUbigeo(this.departamentoFormControl.value, NivelUbigeo.TRES);
  }
