...
export class RegistroPersonerosComponent extends AuthComponent implements OnInit, OnDestroy {
  ...
  onProcesoChanged(): void {
    if (this.procesoControl) {
      this.procesoControl.valueChanges.subscribe(value => {
        this.isConsulta = true;
      });
    }
  }
  ...
  eliminarPersonero(elemento: any) {
    ...
    if (index > -1) {
      ...
      // this.data = [...this.data]; // Fuerza la actualización de la tabla
      this.dataFiltrado = this.data.filter(item=>item.activo === 1);
 ...
 async agregarPersonero() {
  ...
  guardarInformacion() {
    ...
    this.personeroService.save(request).subscribe(response => {
      if(response.success){
        this.generalService.openDialogoGeneral({
          mensaje: 'Personeros registrados de forma exitosa',
          icon: IconPopType.CONFIRM,
          title: TitlePop.INFORMATION,
          success: false
        });
