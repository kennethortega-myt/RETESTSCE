...
export class ReporteActasContabilizadasComponent extends AuthComponent implements OnInit, OnDestroy{
  ...
  reporteContabilizacionActasCorrecto(response: GenericResponseBean<ActaContabilizadaResumenReporteBean>){
    ...
    if(response.success){
      ...
      this.ELEMENT_DATA2 = [];
      this.ELEMENT_DATA2.push(response.data.votosValidos)
      this.ELEMENT_DATA2.push(response.data.detalleNoValidos[0]);
      this.ELEMENT_DATA2.push(response.data.detalleNoValidos[1]);
      this.ELEMENT_DATA2.push(response.data.votosEmitidos);
