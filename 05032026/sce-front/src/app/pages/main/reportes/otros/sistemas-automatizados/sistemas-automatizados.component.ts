...
import { EstadoMesa } from 'src/app/model/estadoMesaPorTipoReporteBean';
...
@Component({
  ...
  styleUrl: './sistemas-automatizados.component.scss'
})
export class SistemasAutomatizadosComponent extends AuthComponent implements OnInit, OnDestroy{
  ...
  public tituloAlert="Sistemas Automatizados";
  ...
  constructor(
    ...
    private sistemasAutomatizadosService: SistemasAutomatizadosService,
        ...
        .subscribe(
          {
            next: (response) => {
              this.listEstado = response.data
                  .filter( estado => this.estadosPermitidos.some(ep => ep === estado.codigo));;
            }
          }
        );
  }
