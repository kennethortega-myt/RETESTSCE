...
import { AsistenciaMiembrosMesaService } from 'src/app/service/reporte/asistencia-miembros-mesa.service';
import { NivelUbigeo } from 'src/app/model/enum/nivel-ubigeo.enum';

@Component({
  selector: 'app-asistencia-miembro-mesa',
  templateUrl: './asistencia-miembro-mesa.component.html'
})
export class AsistenciaMiembroMesaComponent extends AuthComponent implements OnInit, OnDestroy{
  ...
  constructor(
    ...
    private readonly asistenciaMiembrosMesaService: AsistenciaMiembrosMesaService
  ) {
    super();
  ...
  buscarReporte(): void {
    ...
    this.asistenciaMiembrosMesaService.getReporteAsistenciaMiembrosMesa(this.filtroAsistenciaMiembroMesa)
    .pipe(takeUntilDestroyed(this.destroyRef))
    .subscribe({
      next: (response) => {
      ...
      error: () => {
        this.isShowReporte = false;
        this.utilityService.mensajePopup(this.tituloAlert, "No fue posible obtener el reporte de asistencia de miembros de mesa.", IconPopType.ERROR);
      }
    })
