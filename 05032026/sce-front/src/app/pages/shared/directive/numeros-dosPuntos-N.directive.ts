...
export class NumerosDosPuntosNDirective implements OnInit{
  ...
  let transformedValue = upperValue.replace(/\D/g, '');

  // Validación de horas (00-23)
  if (transformedValue.length >= 2) {
    const hours = parseInt(transformedValue.slice(0, 2), 10);
    if (hours > 23) {
      transformedValue = '23' + transformedValue.slice(2);
    }
  }

  // Validación de minutos (00-59)
  if (transformedValue.length >= 4) {
    const minutes = parseInt(transformedValue.slice(2, 4), 10);
    if (minutes > 59) {
      transformedValue = transformedValue.slice(0, 2) + '59';
    }
  }

  if (transformedValue.length >= 2) {
    transformedValue = transformedValue.slice(0, 2) + (transformedValue.length > 2 ? ':' + transformedValue.slice(2, 4) : '');
  }
