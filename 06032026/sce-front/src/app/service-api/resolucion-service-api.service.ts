...
export class ResolucionServiceApiService {
  ...
  obtenerListProcesamientoManual(): Observable<Array<ActasPorCorregirListBean>> {
    return this.httpClient.get<Array<ActasPorCorregirListBean>>(
      this.urlServidor + Constantes.CB_RESOLUCIONES_CONTROLLER_LIST_PROCESAMIENTO_MANUAL,
      {
        headers:  { 'Authorization': "Bearer " + this.auth.getCToken()}
      });
  }
  ...
  obtenerInfoActaParaProcesamientoManual(idActa: number): Observable<GenericResponseBean<ActaBean>>{
    const param = new HttpParams()
      .set('idActa', idActa);
    return this.httpClient.get<GenericResponseBean<ActaBean>>(
      this.urlServidor + Constantes.CB_RESOLUCIONES_CONTROLLER_INFO_ACTA_PROCESAMIENTO_MANUAL,
      {
        params:param,
        headers:  { 'Authorization': "Bearer " + this.auth.getCToken()}
      });
  ...
  registrarProcesamientoManual(acta: ActaBean): Observable<IGenericInterface<boolean>> {
    return this.httpClient.post<IGenericInterface<boolean>>(
      this.urlServidor + Constantes.CB_RESOLUCIONES_CONTROLLER_REGISTRAR_PROCESAMIENTO_MANUAL,
      acta,
      {
        headers:  { 'Authorization': "Bearer " + this.auth.getCToken()}
      });
