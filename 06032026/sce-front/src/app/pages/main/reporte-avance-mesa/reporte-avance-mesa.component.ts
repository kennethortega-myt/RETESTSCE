...
@Component({
  selector: 'app-reporte-avance-mesa',
  templateUrl: './reporte-avance-mesa.component.html',
  styleUrls: ['./reporte-avance-mesa.component.scss']
})
export class ReporteAvanceMesaComponent extends AuthComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public pdfBlob: Blob;
  public tituloAlert="Avance de mesa por mesa";

  public usuario: Usuario;
  constructor(
    private generalService : GeneralService,
    private reporteAvanceMesaService: ReporteAvanceMesaService,
  ...
  onChangeMesa(){
    const numeroMesaControl = this.formGroupReporte.get('numeroMesaFormControl');
    if (numeroMesaControl){
      numeroMesaControl.valueChanges
        .subscribe(mesa => {
          this.isShowReporte = false;
        })
      ;
  ...
  onCentroComputoChanged(): void{
    const centroComputControl = this.formGroupReporte.get('centroComputoFormControl');
    if (centroComputControl){
      centroComputControl.valueChanges
        .pipe(
          tap(() => {
  ...
  onDepartamentoChanged(){
    const departamentoControl = this.formGroupReporte.get('departamentoFormControl');
  ...
  onDistritoChanged():void{
    const distritoControl = this.formGroupReporte.get('distritoFormControl');
    if (distritoControl){
      distritoControl.valueChanges
        .subscribe(mesa => {
          this.isShowReporte = false;
  ...
  onProvinciaChanged(){
    const provinciaControl = this.formGroupReporte.get('provinciaFormControl');
    if (provinciaControl){
