...
export class OmisosListaElectoresComponent extends AuthComponent implements OnInit, OnDestroy{
  ...
  async getBlobDefault(){
    let imageUrlLE = '../../../../../assets/img/doc/LE_NULL.png';
    let imageNullBlobLE;
    try{
      let response = await fetch(imageUrlLE);
      imageNullBlobLE = await response.blob();
    } catch(error){
      console.error(error);
    }

    return imageNullBlobLE;
  }
  ...
  async loadPage(pageIndex: number): Promise<void> {
    ...
    sessionStorage.setItem('loading', 'true');
    // if (this.data.paginas[this.currentPage].existeObservacion) {
    //   this.utilityService.mensajePopup(this.tituloAlert,"Revise las observaciones antes de continuar.",IconPopType.ALERT);
    // }
    ...
    if (typeof page.archivoObservacionPng === 'undefined') {
    ...
    await Promise.all(
      page.secciones.map(async (seccion, j) => {
        if (typeof seccion.archivoSeccionPng === 'undefined') {
  ...
  requisitoMinimoParaContinuar(): boolean{
    if (!this.selectedObservaciones.has(this.currentPage)){
      return false;
    }
    return true;
  }
  ...
  obtenerObservacionesFaltantes(): number[] {
    const observacionesFaltantes: number[] = [];
    for (let i = 0; i < this.getTotalPages(); i++) {
      if (!this.selectedObservaciones.has(i)) {
        observacionesFaltantes.push(i+1);
      }
    }
    return observacionesFaltantes;
  }
  ...
  grabarOmisosLE(){
    let dataGuardar = JSON.parse(JSON.stringify(this.data));
