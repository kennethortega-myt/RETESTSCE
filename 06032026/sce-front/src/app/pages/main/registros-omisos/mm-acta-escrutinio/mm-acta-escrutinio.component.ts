...
export class MmActaEscrutinioComponent extends AuthComponent implements OnInit, OnDestroy {
  ...
  public tituloAlert = "Registro de acta de escrutinio - Miembros de mesa";
  ...
  constructor(private generalService: GeneralService,
              ...
              private miembroMesaEscrutinioService: MiembroMesaEscrutinioService,
  ...
  guardarInformacion() {
    ...
    if(!request.documentoIdentidadPresidente && !request.documentoIdentidadSecretario && !request.documentoIdentidadTercerMiembro){
      const dialog = this.generalService.openDialogoGeneral({
        mensaje: 'Se ha registrado menos de 03 MM. ¿Desea continuar?',
        icon: IconPopType.ALERT,
        title: TitlePop.INFORMATION,
        isQuestion: true,
        success: true
  ...
  saveMme(request: IMiembroMesaEscrutinioRequest){
    ...
    this.miembroMesaEscrutinioService.save(request).subscribe(response => {
      if (response.success) {
        this.generalService.openDialogoGeneral({
          mensaje: 'Miembros de mesa del acta de escrutinio registrados de forma exitosa',
  ...
  resetComponent(isSaltar:boolean) {
    ...
    //this.procesoControl.reset();
    //this.procesoControl.setValue(0);
    this.isConsulta = true;
