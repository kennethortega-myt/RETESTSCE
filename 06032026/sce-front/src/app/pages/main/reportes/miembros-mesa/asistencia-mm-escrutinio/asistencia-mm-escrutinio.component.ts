...
import { AsistenciaMmEscrutinioService } from 'src/app/service/reporte/asistencia-mm-escrutinio.service';
import { NivelUbigeo } from 'src/app/model/enum/nivel-ubigeo.enum';


@Component({
  selector: 'app-asistencia-mm-escrutinio',
  templateUrl: './asistencia-mm-escrutinio.component.html'
})
export class AsistenciaMmEscrutinioComponent extends AuthComponent implements OnInit, OnDestroy{
  ...
  public tituloAlert="Asistencia de Miembros de Mesa";
  ...
  constructor(
    ...
    private readonly asistenciaMmEscrutinioService: AsistenciaMmEscrutinioService
  ) {
    super();
  ...
  buscarReporte(): void {
    ...
    this.asistenciaMmEscrutinioService.getReporteAsistenciaMMEscrutinio(this.filtroAsistenciaMiembroMesa)
    .pipe(takeUntilDestroyed(this.destroyRef))
    .subscribe({
      next: (response) => {
      ...
      error: () => {
        this.isShowReporte = false;
        this.utilityService.mensajePopup(this.tituloAlert, "No fue posible obtener el reporte de asistencia de miembros de mesa según actas de escrutinio.", IconPopType.ERROR);
      }
    })
