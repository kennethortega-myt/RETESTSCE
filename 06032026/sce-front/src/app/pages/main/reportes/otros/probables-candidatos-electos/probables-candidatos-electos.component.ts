...
export class ProbablesCandidatosElectosComponent extends AuthComponent implements OnInit {
  ...
  public listCargos: Array<DetCatalogoEstructuraBean>;
  ...
  obtenerEstados() {
    this.monitoreoService.obtenerTipoEstadoReporte(this.proceso.nombreEsquemaPrincipal,
            'n_cargo',
            this.proceso.acronimo)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe(
          {
            next: (response) => {
  ...
  get filtroProbablesCandidatos(): FiltroProbablesCandidatos {
    ...
    return {
      ...
      cargo: this.form.get('cargo').value.idCodigo,
