...
import { AvanceDigitalizacionResolucionService } from 'src/app/service/reporte/avance-digitalizacion-resolucion.service';
...
export class DigitalizacionComponent extends AuthComponent implements OnInit, OnDestroy {
  ...
  public tituloAlert="Avance de digitalización de resoluciones";
  ...
  constructor(
    ...
    private readonly avanceDigitalizacionResolucionService: AvanceDigitalizacionResolucionService
  ) {
    super();
  ...
  buscarReporte(): void {
    ...
    this.avanceDigitalizacionResolucionService.getReporteAvanceDigitalizacionResolucion(this.filtroActas)
    .pipe(takeUntilDestroyed(this.destroyRef))
    .subscribe(
      {
        next: (response) => {
        ...
        error: () => {
          ...
          this.isShowReporte = false;
          this.utilityService.mensajePopup(this.tituloAlert, "No fue posible obtener el reporte de Avance de digitalización de resoluciones.", IconPopType.ERROR);
        }
      }
    );
  }
  ...
  get filtroActas() {
    ...
    const nivelUbigeoDos = this.form.get('nivelUbigeoDos').value;
    const nivelUbigeoTres = this.form.get('nivelUbigeoTres').value;

    const departamento = nivelUbigeoUno === '0' || nivelUbigeoUno === 0 ? 'TODOS' :
      this.listNivelUbigeoUno.find(d => d.id == nivelUbigeoUno)?.nombre || 'TODOS';

    const provincia = nivelUbigeoDos === '0' || nivelUbigeoDos === 0 ? 'TODOS' :
      this.listNivelUbigeoDos.find(p => p.id == nivelUbigeoDos)?.nombre || 'TODOS';

    const distrito = nivelUbigeoTres === '0' || nivelUbigeoTres === 0 ? 'TODOS' :
      this.listNivelUbigeoTres.find(d => d.id == nivelUbigeoTres)?.nombre || 'TODOS';
