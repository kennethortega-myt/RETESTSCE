...
class PrimaryCaps(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, num_features, kernel_size, num_capsules, capsule_dim, stride=1, **kwargs):
        super(PrimaryCaps, self).__init__(**kwargs)
        self.num_features = num_features    # F
        self.kernel_size = kernel_size      # K
        self.num_capsules = num_capsules    # N
        self.capsule_dim = capsule_dim      # D
        self.stride = stride                # s
        
    def build(self, input_shape):    
        self.dw_conv2d = tf.keras.layers.Conv2D(
            self.num_features,
            self.kernel_size,
            self.stride,
            activation='linear',
    ...
    def call(self, inputs):      
        ...
        x = tf.keras.layers.Reshape((self.num_capsules, self.capsule_dim))(x)
        x = Squash()(x)
        return x
...
class FCCaps(tf.keras.layers.Layer):
    ...
    call(inputs)
        compute the primary capsule layer
    """
    def __init__(self, num_capsules, capsule_dim, kernel_initializer='he_normal', **kwargs):
